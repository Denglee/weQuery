<template>
    <div class="loan-main">
        <!--导航-->
        <!--<van-swipe :loop="false" :show-indicators="false" :width="swiper.swiperWid" :height="swiper.swiperHei" >
            <van-swipe-item v-for="(item,index) in navArr" :key="index"
                      :class="loanIndex == index ? 'activeNav':'' " @click="swiperNav(item,index)">
                {{item.name}}
            </van-swipe-item>
        </van-swipe>-->
        ...
        <van-row gutter="12" v-else>
            <van-col span="8" v-for="(item,index) in loanArr" :ket="index">
                <div  class="fire-list" @click="goLoanDetails">
                    ...
                    &lt;!&ndash;<div class="fire-month fire-quoto">{{item.maxQuota}}</div>&ndash;&gt;
