<template>
	<div class="reslist-box">
		<div class="resSearch-box">
			...
			<ul class="picker-ul">
                <li>
                    <div class="picker-item picker-item2">
                ...
                <li>
                    <div class="picker-item picker-item3">
                        <van-field
                            readonly
                            clickable
                            name="picker"
                    ...
                    <div class="picker-item picker-item3">
                        <van-field
                            ...
                            clickable
                            name="picker"
                            :value="ModeTypeArr.ModeTypeName"
                            placeholder="选择贷款类型"
                            @click="ModeTypeArr.ModeShowType = true" />
                        <van-popup v-model="ModeTypeArr.ModeShowType" position="bottom">
                            <van-picker
                                show-toolbar
                                :columns="navArr"
                                value-key="label"
                                @confirm="ModeTypeConfirm"
                                @cancel="ModeTypeArr.ModeShowType = false" />
                        </van-popup>
                    </div>
                </li>
...
<script>
import { getHistoryProductList,getModelTypeApi } from "@/assets/js/api"; /*引用 接口*/
export default {
	name: "matchResult",
	data() {
		return {
	...
	methods: {
        /*贷款类型*/
        ModeTypeConfirm(value, index){
            console.log(value)
            if (value.name == "所有还款方式") {
                this.isSelect = false;
                this.ModeTypeArr.ModeShowType = false;
            } else {
                this.isSelect = true;
                console.log(value);
