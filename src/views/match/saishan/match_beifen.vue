		...
		<div v-show="showState.matchShow">
			...
			<div v-if="showStatePage.basicShow">
				...
				<ul>
					...
					<!-- 单位/公司是否可考察 除了车贷 -->
					<li class="info-item"
					    v-show="this.tabBasicIndex !=3 && userChecked2.zylx == 2 && userChecked.yyzznx != 1  && userChecked.yyzznx != -1">
					...
					<li class="info-item"
					    v-show="this.tabBasicIndex ==1 && userChecked.yyzznx > 4  && userChecked.yyzznx != -1">
							...
							<li class="info-item">
								...
								<van-radio-group v-model="userChecked.gjjjs" class="info-box">
									<van-radio :name="item.name" v-for="(item,index) in userInfo.gjjjs" :key="item.id">
										<template #icon="props">
											<div class="infoRadio" :class="props.checked ? activeRadio : ' ' ">{{ item.value }}
											</div>
										</template>
									</van-radio>
								</van-radio-group>
			...
			<div v-else-if="showStatePage.assetsShow">
				...
				<van-button class="btn-next" @click="btnNext('creditInfo')" v-show="showNextBtn.assetsNext">下一步</van-button>
