...
router.beforeEach((to, from, next) => {
  ...
  const openid = ls.getItem("openid2");
	...
	const urlObj = GetRequest(window.location.href); // 截取code
	if(urlObj.proxyOpenid){
		localStorage.setItem("proxyOpenid", urlObj.proxyOpenid);
	}
	console.log('proxyOpenid',urlObj.proxyOpenid);

  console.log(typeof (openid));
  const oauth_token = ls.getItem("oauth_token");
  console.log(oauth_token);
  // let url = `https://open.w`eixin.qq.com/connect/oauth2/authorize?appid=wxdcf6df398c3ce766&redirect_uri=http%3a%2f%2fwww.jierong123.com%2fdist%2findex.html%23%2flogin&response_type=code&scope=snsapi_userinfo&state=jierong#wechat_redirect`;

	const loginUrl= "http%3a%2f%2fwww.jierong123.com%2fdist%2f%23%2flogin";
	// const loginUrl= "http%3A%2F%2Fwww.jierong123.com%2Fdist%2Ftest%2F%23%2Flogin";
	let url = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxdcf6df398c3ce766&redirect_uri=${loginUrl}
	&response_type=code&scope=snsapi_userinfo&state=jierong#wechat_redirect`;
